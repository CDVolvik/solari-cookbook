...
const registry: Site[] = JSON.parse(
  await readFile(path.resolve(values.sites!), "utf8"),
)
  ...
  const workers = Array.from(
    ...
    async () => {
      for (let task = queue.shift(); task; task = queue.shift()) {
        ...
        results.push(result)
        console.log(
          `${result.verdict.padEnd(15)} ${result.site} — ${result.detail}`,
        )
      }
    },
  )
