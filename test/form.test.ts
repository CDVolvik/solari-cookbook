...
describe('classifyField', () => {
  ...
  it('never fills a hidden or invisible field', () => {
    expect(classifyField(field({ type: 'hidden', name: 'email' }))).toBe('honeypot')
    expect(classifyField(field({ visible: false, name: 'email' }))).toBe('honeypot')
  })
...
describe('computeVerdict', () => {
  ...
  it('calls a confirmed-but-undelivered form a silent failure', () => {
    expect(
      computeVerdict({ ...base, submitted: true, confirmed: true, delivered: false }),
    ).toBe('silent-failure')
  })

  it('trusts delivery over anything the page claimed', () => {
    expect(
      computeVerdict({ ...base, submitted: true, confirmed: false, delivered: true }),
    ).toBe('delivered')
  })

  it('does not overstate when delivery was never checked', () => {
    expect(computeVerdict({ ...base, submitted: true, confirmed: true })).toBe('confirmed')
...
describe('didConfirm', () => {
  ...
  it('accepts a confirmation that appeared after submitting', () => {
    expect(didConfirm('Contact us', 'Thank you, we received it', false)).toBe(true)
  })
